...
export function Lightbox({ fotos }: { fotos: FotoProgresso[] }) {
  ...
  const abrir = (i: number) => { setIndiceAberto(i); resetarZoom(); };
  const fechar = useCallback(() => { setIndiceAberto(null); resetarZoom(); }, [resetarZoom]);
  ...
  async function downloadFoto(url: string) {
    try {
      ...
      link.download = `atelie-${indiceAberto! + 1}.${blob.type.split('/')[1] || 'jpg'}`;
      link.click();
      ...
      {fotoAtual && (
        <div className="fixed inset-0 z-50 bg-black/95 flex flex-col select-none animate-fadeIn" role="dialog" aria-modal="true">
          <div className="flex items-center justify-between px-4 py-3 text-atelie-texto text-sm shrink-0">
            <span className="text-atelie-textoMuted font-mono">{indiceAberto! + 1} / {fotos.length}</span>
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => downloadFoto(fotoAtual.url_foto)} aria-label="Baixar"
          ...
          <div ref={containerRef}
            ...
            {indiceAberto! > 0 && (
              <button onClick={(e) => { e.stopPropagation(); irPara(indiceAberto! - 1); }}
            ...
            <div className="relative w-full h-full max-w-5xl max-h-[75vh] mx-auto"
              ...
              <Image src={fotoAtual.url_foto} alt={fotoAtual.legenda ?? ''} fill className="object-contain pointer-events-none" draggable={false} priority />
            </div>
          </div>
          ...
          {fotos.length > 1 && (
            <div className="flex gap-2 px-4 pb-4 overflow-x-auto shrink-0 justify-center">
              {fotos.map((foto, i) => (
                <button key={foto.id} onClick={() => irPara(i)}
                  className={`relative w-14 h-14 shrink-0 rounded-md overflow-hidden border-2 transition-all duration-200 ${i === indiceAberto ? 'border-atelie-dourado shadow-dourado' : 'border-transparent opacity-50 hover:opacity-100'}`}>
